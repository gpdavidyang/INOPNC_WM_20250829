...
export function calculateEnhancedSalary(
  ...
  // 1. 기본 급여 계산 (공수 기준)
  const total_hours = labor_hours * 8 // 공수를 시간으로 변환
  const regular_hours = Math.min(total_hours, 8)
  const overtime_hours = Math.max(total_hours - 8, 0)
  
  const base_pay = (regular_hours / 8) * daily_rate
  const overtime_pay = (overtime_hours / 8) * daily_rate * 1.5 // 연장근무 1.5배
  const gross_pay = base_pay + overtime_pay + bonus_pay
 ...
 * 월별 급여 집계 계산
 */
export function calculateMonthlySalaryAggregate(
  daily_records: EnhancedSalaryCalculationResult[]
): {
  ...
  work_days: number
  employment_type_breakdown: Record<EmploymentType, {
    count: number
    total_gross: number
    total_tax: number
    total_net: number
  ...
  const totals = daily_records.reduce((acc, record) => {
