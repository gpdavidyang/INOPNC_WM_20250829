...
async function testRLSPolicies() {
  ...
  for (const user of testUsers) {
    ...
    console.log(`   파트너사 ID: ${user.partner_company_id || '없음'}`)
    ...
    try {
      ...
      if (profilesError) {
        ...
        if (user.role === 'customer_manager' && user.partner_company_id) {
          const partnerProfiles = profiles?.filter(p => p.partner_company_id === user.partner_company_id) || []
  ...
  if (adminResult && partnerResult && workerResult) {
    console.log(`• 관리자는 모든 데이터에 접근 가능: ${adminResult.results.sites > 0 ? '✅' : '❌'}`)
    console.log(`• 파트너 매니저도 현재는 모든 데이터 접근 가능 (RLS 적용 필요): ${partnerResult.results.sites > 0 ? '⚠️' : '❌'}`)
    console.log(`• 작업자도 모든 데이터 접근 가능: ${workerResult.results.sites > 0 ? '✅' : '❌'}`)
    
    if (partnerResult.results.sites === adminResult.results.sites) {
      console.log('⚠️  customer_manager가 여전히 모든 현장에 접근할 수 있습니다. RLS 정책이 제대로 적용되지 않았을 수 있습니다.')
    }
  }
}
