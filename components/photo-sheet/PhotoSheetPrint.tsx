...
export default function PhotoSheetPrint({
  ...
  const { metaMm, gapMm } = useMemo(() => {
    ...
    if (!templateMode) {
      if (perCellCaption) {
        ...
        if (rows === 2 && cols === 2) return { metaMm: 0, gapMm: 2.5 }
      }
  ...
  const pageGapMm = perCellCaption && rows === 2 && cols === 2 ? 3 : 4
  ...
  const pageVars = useMemo(() => {
    ...
    if (perCellCaption) {
      ...
      else if (rows === 2 && cols === 2) cellCap = '24mm'
    }
    return {
...
.footer { display: grid; grid-template-columns: 1fr 1fr 1fr; font-size: 9pt; color: #333; align-items: center; margin-top: 4mm; }
