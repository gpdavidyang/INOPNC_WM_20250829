...
const mapDocumentsToAttachments = (documents?: Record<string, any[]>): UnifiedAttachments => {
  const mapCategory = (items: any[] | undefined, type: UnifiedAttachment['type']) =>
    (items || []).map(item => ({
...
const mapTaskGroups = (dailyReport: any): UnifiedTaskGroup[] | undefined => {
  if (!Array.isArray(dailyReport?.work_logs) || dailyReport.work_logs.length === 0) return undefined
...
export const integratedResponseToUnifiedReport = (
  ...
  const npcUsage = response.daily_report
    ? {
