...
export const useDocumentState = () => {
  ...
  const updateSelectedDocuments = useCallback((documents: string[]) => {
  ...
  const updateActiveTab = useCallback((tab: 'mine' | 'shared') => {
    setDocumentState(prev => ({
      ...prev,
      activeTab: tab
    }))
  ...
  const updateFontSize = useCallback((size: 'fs-100' | 'fs-150') => {
    setDocumentState(prev => ({
      ...prev,
      fontSize: size
    }))
  ...
  const updateDeleteMode = useCallback((mode: boolean) => {
    setDocumentState(prev => ({
      ...prev,
      deleteMode: mode
    }))
...
export const useUserPreferences = () => {
  ...
  // 폰트 크기 업데이트
  const updateFontSize = useCallback((fontSize: 'fs-100' | 'fs-150') => {
    setPreferences(prev => ({
      ...prev,
      fontSize
    }))
...
export const useUploadHistory = () => {
  const [uploadHistory, setUploadHistory, removeUploadHistory] = useLocalStorage<UploadHistoryItem[]>(
    'uploadHistory',
    []
  )

  // 업로드 기록 추가
  const addUploadRecord = useCallback((record: Omit<UploadHistoryItem, 'uploadDate'>) => {
  ...
  // 특정 문서의 업로드 기록
  const getDocumentUploads = useCallback((documentId: string) => {
  ...
  const cleanOldRecords = useCallback(() => {
    ...
    setUploadHistory(prev => 
      prev.filter(item => new Date(item.uploadDate) > thirtyDaysAgo)
    )
  }, [setUploadHistory])
