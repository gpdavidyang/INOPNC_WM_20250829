...
export default function DocumentManagement({ profile }: DocumentManagementProps) {
  ...
  const handleBulkApproval = (action: 'approve' | 'reject') => async (documentIds: string[]) => {
    const comments = prompt(
      `${action === 'approve' ? '승인' : '거부'} 사유를 입력하세요 (선택사항):`
    )
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (site: { name: string } | undefined) => (
        <span className="text-sm text-gray-900 dark:text-gray-100">{site?.name || '전체'}</span>
      ),
    },
    {
      key: 'created_at',
    ...
    <div className="space-y-4">
      ...
      {viewMode === 'integrated' && integratedDocuments ? (
        ...
        <div className="space-y-6">
          ...
          {integratedDocuments.documents_by_category &&
            Object.entries(integratedDocuments.documents_by_category).map(
              ([category, docs]: [string, any[]]) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`bg-${config.color}-50 dark:bg-${config.color}-900/20 p-6 border-b border-${config.color}-200 dark:border-${config.color}-700`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <div>
                            <h3
                              className={`text-lg font-semibold text-${config.color}-900 dark:text-${config.color}-100`}
                            >
                              {config.name}
                            </h3>
                            <p
                              className={`text-sm text-${config.color}-700 dark:text-${config.color}-300`}
                            >
                              {config.description}
                            </p>
                          </div>
                        ...
                        <div
                          className={`bg-${config.color}-100 dark:bg-${config.color}-800 px-3 py-1 rounded-full`}
                        >
                          <span
                            className={`text-sm font-medium text-${config.color}-800 dark:text-${config.color}-200`}
                          >
                            {docs.length}개 문서
                          </span>
                        </div>
                    ...
                    <div className="p-6">
                      {docs.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {docs.slice(0, 6).map(doc => (
                            ...
                            >
                              <div className="flex items-start space-x-3">
                                <div
                                  className={`p-2 bg-${config.color}-100 dark:bg-${config.color}-800 rounded-lg`}
                                >
                        ...
                        <div className="text-center py-12">
                          <div
                            className={`p-4 bg-${config.color}-100 dark:bg-${config.color}-800 rounded-full inline-block mb-4`}
                          >
                      ...
                      {docs.length > 6 && (
                        <div className="mt-4 text-center">
                          <button
                            className={`text-${config.color}-600 hover:text-${config.color}-800 dark:text-${config.color}-400 dark:hover:text-${config.color}-300 text-sm font-medium`}
                          >
