...
export async function deleteBackupConfig(
  ...
  try {
    ...
    const { error } = await (supabase.from('backup_configs').delete().eq('id', id) as unknown)
...
export async function getBackupJobs(
  ...
  try {
    ...
    let query = (supabase.from('backup_jobs') as unknown)
      .select(
        `
        *,
        backup_configs(name)
    ...
    return {
      success: false,
      error:
        error instanceof AppError ? error.message : '백업 작업 목록을 불러오는데 실패했습니다.',
    }
  }
}
...
export async function getBackupJob(jobId: string): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof AppError ? error.message : '백업 작업 정보를 불러오는데 실패했습니다.',
    }
  }
}
...
export async function getBackupStats(): Promise<{
  ...
  try {
    ...
    const completedJobs =
      jobs?.filter((j: unknown) => j.status === 'completed' && j.started_at && j.completed_at) || []
...
export async function createBackupSchedule(
  ...
  try {
    ...
    const { data, error } = await (
      supabase.from('backup_schedules').select('*').eq('id', result.id) as unknown
    ).single()
