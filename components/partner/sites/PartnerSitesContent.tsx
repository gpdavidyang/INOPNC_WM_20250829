...
import {
  SiteDetailSheet,
  type SiteAssignmentSummary,
} from '@/components/admin/sites/SiteDetailSheet'

interface PartnerSitesContentProps {
  pageSize: number
  initialSites?: Site[]
  initialTotal?: number
  initialPages?: number
...
export function PartnerSitesContent({
  pageSize,
  initialSites = [],
  initialTotal = 0,
  initialPages = 1,
  initialLoadErrored = false,
}: PartnerSitesContentProps) {
  ...
  const fetchSites = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch(`/api/partner/sites?${params.toString()}`, {
          cache: 'no-store',
        })
        ...
        console.error('Failed to fetch partner sites', err)
  ...
  // Auto-load first page if no initial data provided
  useEffect(() => {
  ...
  const fetchAssignments = useCallback(async (site: Site) => {
    ...
    try {
      const response = await fetch(`/api/partner/sites/${site.id}/assignments`, {
        cache: 'no-store',
      })
      const payload = await response.json()
...
export default PartnerSitesContent
