  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <Button
          variant="outline"
          className={`gap-1.5 ${touchMode === 'glove' ? 'h-10 px-3' : 'h-9 px-2.5'}`}
        >
          <Filter className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
        ...
        <Button
          ...
          className={`gap-1.5 ${touchMode === 'glove' ? 'h-10 px-3' : 'h-9 px-2.5'}`}
        >
          <ArrowUpDown className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
          <span className={getFullTypographyClass('caption', 'sm', isLargeFont)}>정렬</span>
        </Button>
      </div>

      {/* Main Content Tabs - Compact (Brand) */}
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
        <TabsList className="w-full" fill>
          ...
          >
            <Package className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
            <span className={`${getFullTypographyClass('caption', 'xs', isLargeFont)} text-center`}>
              NPC-1000
            </span>
          </TabsTrigger>
          <TabsTrigger
          ...
          >
            <Building2 className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
            <span className={`${getFullTypographyClass('caption', 'xs', isLargeFont)} text-center`}>
              재고
            </span>
          </TabsTrigger>
          ...
          >
            <FileText className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
            <span className={`${getFullTypographyClass('caption', 'xs', isLargeFont)} text-center`}>
              요청
            </span>
          </TabsTrigger>
          ...
          >
            <Truck className={`${touchMode === 'glove' ? 'h-4 w-4' : 'h-3.5 w-3.5'}`} />
            <span className={`${getFullTypographyClass('caption', 'xs', isLargeFont)} text-center`}>
              입출고
            </span>
          </TabsTrigger>
        ...
        <TabsContent value="npc1000" className="space-y-4">
          <NPC1000Management currentSite={currentSite} currentUser={currentUser} />
        </TabsContent>
