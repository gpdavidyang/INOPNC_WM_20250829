...
for (let i = 3; i <= 12; i++) {
  mySites.push({
    id: i,
    name: `테스트 현장 ${i}구역`,
    status: i % 2 === 0 ? 'ing' : 'wait',
    days: i * 5,
    mp: i * 10,
    address: `서울시 강남구 삼성동 ${i}번지`,
    worker: i + 2,
    manager: `김관리${i}`,
    safety: `이안전${i}`,
    affil: i % 3 === 0 ? '본사' : '서울지사',
    lastUpdate: `2025-12-24 1${i}:00`,
    hasDraw: i % 2 === 0,
    hasPhoto: i % 3 === 0,
    hasPTW: i % 4 === 0,
    hasLog: true,
...
function Dashboard({ onNavigate }: { onNavigate: (tab: Tab) => void }) {
  ...
  const handleLoadMore = () => {
    if (visibleCount >= mySites.length) {
  ...
  return (
    <div ref={wrapperRef} className="dashboard-wrapper h-full overflow-y-auto no-scrollbar">
      {/* Quick Menu */}
      ...
      {/* Site Cards List */}
      <div>
        {mySites.slice(0, visibleCount).map(s => {
        ...
        {mySites.length > initialCount && (
          <button className="btn-load-more" onClick={handleLoadMore}>
            <span>{visibleCount >= mySites.length ? '접기' : '더 보기'}</span>
            {visibleCount >= mySites.length ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </button>
        )}
      </div>
    </div>
