...
const DATA_POOL = {
  ...
  작업공간: ['지하1층', '지하2층', '1층', '2층', '3층', '4층', '5층', '옥상', '기계실', '전기실', '화장실', '계단실'],
  작업구간: ['A구역', 'B구역', 'C구역', 'D구역', '동측', '서측', '남측', '북측', '중앙부', '모서리부'],
  작업내용: [
    '균열보수', '방수작업', '타일시공', '페인트작업', '철근배근', 
    '콘크리트타설', '거푸집설치', '단열재시공', '방화재시공', '마감작업'
  ],
  시공업체: [
    '대한건설', '삼성건설', '현대건설', '롯데건설', 'GS건설', 
    '두산건설', '포스코건설', 'SK건설', '한화건설', '태영건설'
  ],
  작업자: [
    '김현수', '이민호', '박지영', '정수연', '최동욱', 
    '한소영', '임재현', '송미경', '윤태호', '장혜진'
...
function main() {
  ...
  photoGrids.forEach((grid, index) => {
    ...
    console.log(`   부재: ${grid.metadata.부재명} | 공간: ${grid.metadata.작업공간} | 구간: ${grid.metadata.작업구간}`)
    console.log(`   작업: ${grid.metadata.작업내용} | 업체: ${grid.metadata.시공업체} | 담당: ${grid.metadata.작업자}`)
