  ...
  try {
    ...
    for (const worker of workers) {
      ...
      const workData = {
        ...
        total_overtime_hours: attendance.reduce(
      ...
      const total_deductions =
        taxInfo.tax_deduction +
        taxInfo.national_pension +
        taxInfo.health_insurance +
        taxInfo.employment_insurance
    ...
    results
      ...
      .forEach(r => {
        console.log(
          `  - ${r.name} (${r.salary_type}): ${r.work_days}일 근무, ${r.net_pay?.toLocaleString()}원`
        )
      })
...
generateAllPayslips(year, month)
  .then(results => {
    console.log('\n🎉 모든 작업 완료!')
    process.exit(0)
  })
