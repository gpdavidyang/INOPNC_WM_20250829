...
import { DocumentGroup } from '@inopnc/shared/types'
import { X, Download, Plus, Minus, Hand, Check, Camera } from 'lucide-react'
...
export const ReportEditor: React.FC<ReportEditorProps> = ({
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const imageContainers = Array.from(clone.querySelectorAll('.border-gray-300'))
  ...
  return (
    <div className="fixed inset-0 bg-[#121212] z-[9999] flex flex-col text-white">
      {/* Header */}
      <header className="h-[60px] bg-black flex items-center justify-between px-4 border-b border-gray-800 shrink-0">
        <button onClick={onClose} className="p-2">
          <X size={24} />
        </button>
        <div className="text-center">
          <div className="text-base font-bold">{siteName} 보고서</div>
          <div className="text-[10px] text-gray-400">PDF 저장 및 미리보기</div>
        </div>
      ...
      <div
        ref={viewportRef}
        className={`flex-1 relative overflow-hidden bg-[#1a1a1a] flex justify-center items-start touch-none ${isPanning ? 'cursor-grabbing' : 'cursor-grab'}`}
      ...
      >
        <div
          ref={contentRef}
          className="bg-white text-black w-[210mm] min-h-[297mm] p-[20mm_10mm] shadow-2xl origin-top transition-transform duration-75"
          style={{
          ...
          <table className="w-full border-collapse border-2 border-black table-fixed">
            ...
            <tbody>
              {validItems.length > 0 ? (
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="border border-black p-2">
                        <div
                          className="w-[100px] h-[80px] border border-gray-300 bg-gray-50 mx-auto flex items-center justify-center relative cursor-pointer group"
                        ...
                        >
                          {imgBefore ? (
                            ...
                            <span className="text-gray-400 text-xs photo-upload-placeholder">
                              사진등록
                            </span>
                          )}
                        </div>
                ...
                <tr>
                  <td colSpan={6} className="border border-black p-8 text-center text-gray-400">
                    데이터가 없습니다.
                  </td>
                </tr>
              )}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-[#222] border border-[#333] rounded-full px-6 py-3 flex gap-6 shadow-xl">
        <button
          onClick={() => setZoom(z => Math.max(0.3, z - 0.1))}
          className="flex flex-col items-center gap-1 text-[10px] text-gray-300 hover:text-white"
        >
          <Minus size={18} /> 축소
        </button>
        <button
          onClick={() => setIsPanning(!isPanning)}
          className={`flex flex-col items-center gap-1 text-[10px] ${isPanning ? 'text-primary font-bold' : 'text-gray-300 hover:text-white'}`}
        >
          <Hand size={18} /> 이동
        </button>
        <button
