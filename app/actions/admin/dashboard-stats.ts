  ...
  return withAdminAuth(async (supabase, profile) => {
    try {
      ...
      try {
        ...
        console.warn('[dashboard-stats] Failed to fetch totalUsers:', err)
      }

      // Active sites count
      let activeSites = 0
      try {
        activeSites = restrictedOrgId
        ...
        console.warn('[dashboard-stats] Failed to fetch activeSites:', err)
      }
      ...
      try {
        ...
        console.warn('[dashboard-stats] Failed to fetch todayReports:', err)
      }
      ...
      try {
        ...
        console.warn('[dashboard-stats] Failed to fetch recentReports:', err)
      }

      recentReportsData.forEach(report => {
        const profileData = report.created_by_profile as { full_name?: string } | undefined
      ...
      try {
        ...
        console.warn('[dashboard-stats] Failed to fetch recentUsers:', err)
      }
      ...
      try {
        ...
        console.warn('[dashboard-stats] Failed to fetch photos:', err)
      }
