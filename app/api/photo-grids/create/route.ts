...
export async function POST(request: NextRequest) {
  console.log('POST /api/photo-grids/create - Request received')
  ...
  try {
    ...
    if (!user) {
      console.log('POST /api/photo-grids/create - Unauthorized user')
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    console.log('POST /api/photo-grids/create - User authenticated:', user.id)
    
    let formData: FormData
    try {
      formData = await request.formData()
      console.log('POST /api/photo-grids/create - FormData parsed successfully')
    } catch (formError) {
      console.error('POST /api/photo-grids/create - Error parsing FormData:', formError)
    ...
    console.error('Error in POST /api/photo-grids/create:', error)
