...
export class SalaryPDFGenerator {
  ...
  private drawHeader(data: SalaryPDFData) {
    ...
    this.pdf.text(`${data.salaryYear}년 ${data.salaryMonth}월분`, pageWidth - 15, 45, { align: 'right' })
  }
  ...
  private drawSalaryDetails(data: SalaryPDFData, startY: number) {
    ...
    const rows = [
      ['기본급', `${calculation.details.workHours}시간 × ${this.formatCurrency(calculation.details.hourlyRate || 0)}`, this.formatCurrency(calculation.basePay)],
    ]
    
    if (calculation.overtimePay > 0) {
      rows.push([
        '연장수당', 
        `${calculation.details.overtimeHours}시간 × ${this.formatCurrency(calculation.details.overtimeRate || 0)}`, 
        this.formatCurrency(calculation.overtimePay)
      ])
    }
    
    if (calculation.bonuses > 0) {
      rows.push(['보너스', '', this.formatCurrency(calculation.bonuses)])
    }
    ...
    this.pdf.text(this.formatCurrency(calculation.netPay), pageWidth - 20, currentY + 2, { align: 'right' })
  ...
  private drawFooter(data: SalaryPDFData) {
    ...
    // Signature area
    this.pdf.text('직원 서명: _________________', pageWidth - 80, pageHeight - 30, { align: 'right' })
