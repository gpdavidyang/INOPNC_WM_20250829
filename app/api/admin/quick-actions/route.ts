...
export async function GET() {
  try {
    ...
    if (error) return NextResponse.json({ error: 'Failed to fetch quick actions' }, { status: 500 })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: quickAction, error } = await supabase
      ...
      .select()
      .single()
    if (error) return NextResponse.json({ error: 'Failed to create quick action' }, { status: 500 })
