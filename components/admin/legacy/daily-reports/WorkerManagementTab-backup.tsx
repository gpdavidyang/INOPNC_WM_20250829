...
export default function WorkerManagementTab({ 
      ...
      try {
        ...
        try {
          ...
          if (!sessionUser) {
            ...
            console.log('[WorkerManagementTab] Session valid for:', sessionUser.email)
            setSessionError(null)
          }
        } catch (error) {
          console.error('[WorkerManagementTab] Auth error:', error)
          setSessionError('Authentication error: ' + (error as Error).message)
        }
      } catch (err) {
  ...
  const handleAddWorker = useCallback(async () => {
    ...
    try {
      ...
      if (!sessionUser) {
        console.error('[WorkerManagementTab] Session check failed: no active session')
