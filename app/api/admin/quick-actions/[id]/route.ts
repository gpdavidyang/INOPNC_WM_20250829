...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ quickAction: data })
  } catch (e) {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const supabase = createClient()
    const { title, description, icon_name, link_url, is_active, display_order } =
      await request.json()
    ...
    const { data, error } = await supabase
      .from('quick_actions')
      .update({ title, description, icon_name, link_url, is_active, display_order })
      .eq('id', params.id)
      .select()
      .single()
    if (error || !data)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const supabase = createClient()
    const { error } = await supabase.from('quick_actions').delete().eq('id', params.id)
    if (error) return NextResponse.json({ error: 'Failed to delete quick action' }, { status: 500 })
