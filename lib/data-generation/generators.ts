...
export class ConstructionDataGenerator {
  ...
  generateSite(): ConstructionSite {
    ...
    estimatedCompletion.setMonth(
      estimatedCompletion.getMonth() + faker.number.int({ min: 6, max: 36 })
    )
  ...
  generateAttendanceRecord(userId: string, siteId: string, date: Date) {
    ...
    if (isWeekend || isHoliday) {
      if (faker.datatype.boolean(0.8)) {
        // 80% chance of no work on weekend/holiday
        status = 'absent'
        laborHours = 0
        workHours = 0
      } else {
...
export class PayrollCalculator {
  ...
  private readonly INCOME_TAX_BRACKETS = [
    ...
    { min: 300000000, max: 500000000, rate: 0.4 }, // 40%
  ...
  calculateMonthlyPayroll(
    ...
    const totalGrossPay = regularPay
...
export class MaterialWorkflowManager {
  ...
  generateRequestedMaterials(): RequestedMaterial[] {
    ...
    return selectedMaterials.map(material => {
      ...
      return {
        ...
        supplier_preference: faker.datatype.boolean(0.4)
          ? faker.helpers.arrayElement(this.suppliers).name
          : undefined,
  ...
  generateApprovalWorkflow(
    ...
    const siteManagerApproval: MaterialApproval = {
      ...
      decision_date:
        status !== 'pending'
          ? faker.date.recent({ days: 2 }).toISOString().split('T')[0]
          : undefined,
  ...
  generateDeliveryInfo(
    ...
    return {
      ...
      actual_delivery_date:
        status === 'delivered'
          ? faker.date.recent({ days: 3 }).toISOString().split('T')[0]
          : undefined,
  ...
  generateMaterialRequest(
    ...
    const totalEstimatedCost = requestedMaterials.reduce(
      (sum, material) => sum + material.estimated_total_cost,
      0
    )
    ...
    if (totalEstimatedCost < 500000) {
      // Under 500k KRW - usually approved quickly
      if (random < 0.7) status = 'approved'
      else if (random < 0.85) status = 'ordered'
      else if (random < 0.95) status = 'delivered'
      else status = 'rejected'
    } else if (totalEstimatedCost < 2000000) {
      // 500k-2M KRW - needs more approval
