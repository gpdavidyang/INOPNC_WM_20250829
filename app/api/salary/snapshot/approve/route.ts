...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await approveSalarySnapshot(workerId, year, month, auth.userId)
    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || '승인 실패' },
        { status: 500 }
      )
    }
    return NextResponse.json({ success: true })
  } catch (e: any) {
    console.error('POST /api/salary/snapshot/approve error:', e)
    return NextResponse.json(
