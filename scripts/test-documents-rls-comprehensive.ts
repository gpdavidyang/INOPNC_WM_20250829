...
const testUsers: TestUser[] = [
  ...
  { email: 'manager@inopnc.com', password: 'password123', role: 'site_manager', name: '현장관리자' },
  { email: 'worker@test.com', password: 'password123', role: 'worker', name: '작업자' },
  { email: 'partner@test.com', password: 'password123', role: 'customer_manager', name: '파트너사' }
]
...
async function testDocumentsRLS() {
  ...
  for (const user of testUsers) {
    ...
    if (selectError) {
      ...
      const otherPrivateDocs = allDocs?.filter(doc => doc.owner_id !== userId && !doc.is_public) || []
  ...
  for (const [operation, stats] of operationResults) {
    const total = stats.pass + stats.fail
    const passRate = total > 0 ? ((stats.pass / total) * 100).toFixed(1) : '0'
    console.log(`${operation.padEnd(20)} : ✅ ${stats.pass} / ❌ ${stats.fail} (성공률: ${passRate}%)`)
  }
