...
// Image directories
const beforeDir =
  '/Users/davidyang/workspace/INOPNC_WM_20250829/dy_memo/DY_INOPNC/사진대지용_사진데이터/photos_before_work'
const afterDir =
  '/Users/davidyang/workspace/INOPNC_WM_20250829/dy_memo/DY_INOPNC/사진대지용_사진데이터/photos_after_work'
...
async function uploadImages() {
  try {
    ...
    for (const file of beforeFiles) {
      ...
      const { data, error } = await supabase.storage.from('photo-grids').upload(file, fileContent, {
    ...
    await supabase.storage.from('photo-grids').upload('placeholder-before.jpg', placeholderData, {
      contentType: 'image/png',
      upsert: true,
    })

    await supabase.storage.from('photo-grids').upload('placeholder-after.jpg', placeholderData, {
      contentType: 'image/png',
      upsert: true,
    })
    ...
    for (const grid of photoGrids || []) {
      ...
      if (Object.keys(updates).length > 0) {
        const { error } = await supabase.from('photo_grids').update(updates).eq('id', grid.id)
