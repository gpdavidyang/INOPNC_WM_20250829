...
import { DrawingObject } from '@inopnc/shared/types'
...
const DrawingModal: React.FC<DrawingModalProps> = ({ isOpen, onClose, imageSrc, onSave }) => {
  ...
  const handleStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    // Check if clicking on existing marking to delete it (regardless of tool)
    const foundIdx = [...drawObjects].reverse().findIndex(o => isHit(o, wp.x, wp.y))
    if (foundIdx !== -1 && currentTool !== 'hand') {
  ...
  const handleSave = () => {
    ...
    if (!imgObj) {
      console.log('No imgObj, returning')
      return
    }
    setIsProcessing(true)
    const dataUrl = generateImage()
    console.log('dataUrl:', dataUrl ? 'generated' : 'failed')
    if (dataUrl) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-50 flex flex-col">
        ...
        <div className="h-[52px] bg-header-navy flex items-center justify-between px-3 text-white border-b border-white/10 shrink-0">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 bg-slate-50/10 rounded-lg text-xs"
            >
      ...
      >
        ...
        {textInput && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-40">
            <div className="bg-[var(--bg-surface)] border border-[var(--border)] p-4 rounded-xl shadow-2xl w-[90%] max-w-sm flex flex-col gap-3">
              <span className="font-bold text-lg text-[var(--text-main)]">텍스트 입력</span>
              <input
                autoFocus
                className="border border-[var(--border)] p-2 rounded text-lg outline-none focus:border-primary bg-[var(--bg-input)] text-[var(--text-main)]"
              ...
              <div className="flex gap-2 justify-end mt-2">
                <button
                  ...
                  className="px-4 py-2 bg-[var(--bg-hover)] rounded-lg font-bold text-text-sub"
                >
                  취소
                </button>
                <button
        ...
        {isPreviewOpen && previewUrl && (
          <div className="absolute inset-0 bg-black flex flex-col z-50">
            <div className="h-[52px] bg-black border-b border-slate-800 flex items-center justify-between px-3 shrink-0 text-white">
            ...
            <div
              className={`flex-1 relative overflow-hidden flex items-center justify-center bg-slate-950 touch-none ${isPreviewPanning ? 'cursor-grab active:cursor-grabbing' : ''}`}
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur border border-white/10 px-6 py-2.5 rounded-full flex gap-6 shadow-2xl z-50">
              <button
              ...
              <div className="w-px h-6 bg-slate-50/20 self-center mx-1"></div>
              <button
      ...
      {!isPreviewOpen && (
        <div className="bg-[var(--bg-surface)] pb-safe shrink-0">
          <div className="flex justify-around p-2 border-t border-[var(--border)]">
            {[
              ...
              <button
                ...
                className={`w-11 h-11 rounded-xl border flex flex-col items-center justify-center text-[10px] gap-0.5 transition-colors
                                    ${currentTool === tool.id ? 'bg-primary-bg border-primary text-primary font-bold' : 'bg-[var(--bg-surface)] border-[var(--border)] text-text-sub'}
                                `}
              >
          ...
          <div className="flex items-center gap-2 px-2 py-2 border-t border-slate-100 overflow-x-auto no-scrollbar">
            ...
            <div className="flex items-center gap-1 text-[13px] text-text-sub">
              <button
                onClick={() => setCurrentSize(Math.max(1, currentSize - 5))}
                className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-xs font-bold"
              >
                －
              </button>
              <span className="min-w-[30px] text-center font-medium">{currentSize}</span>
              <button
                onClick={() => setCurrentSize(Math.min(50, currentSize + 5))}
                className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-xs font-bold"
              >
                ＋
              </button>
            ...
            <div className="flex gap-1">
              <button
                onClick={() => setCamera({ ...camera, scale: Math.max(0.5, camera.scale - 0.2) })}
                className="p-1.5 rounded hover:bg-slate-100 transition-colors"
              >
                <ZoomOut size={16} className="text-slate-600" />
              </button>
              <button
                onClick={() => setCamera({ ...camera, scale: Math.min(3, camera.scale + 0.2) })}
                className="p-1.5 rounded hover:bg-slate-100 transition-colors"
              >
              ...
              <button
                onClick={() => {
                  if (imgObj && containerRef.current) {
                    ...
                    const scale = Math.min(cw / imgObj.width, ch / imgObj.height, 1)
                    setCamera({
                      scale,
                      x: (cw - imgObj.width * scale) / 2,
                      y: (ch - imgObj.height * scale) / 2,
                    })
                  }
                }}
                className="px-2 py-1.5 text-xs border rounded flex items-center gap-1 bg-slate-50 text-slate-600"
              >
          ...
          {currentTool === 'stamp' && (
            <div className="flex justify-around p-2 bg-slate-50 gap-2 border-t border-slate-100">
              {['circle', 'square', 'triangle', 'star', 'diagonal'].map(s => (
                <button
                  key={s}
                  onClick={() => setCurrentStamp(s)}
                  className={`p-2 rounded-lg border ${currentStamp === s ? 'bg-primary-bg border-primary text-primary' : 'bg-[var(--bg-surface)] border-[var(--border)]'}`}
                >
                  {s === 'circle' && (
                  ...
                  {s === 'star' && <div className="text-lg">★</div>}
                  {s === 'diagonal' && <div className="text-2xl font-bold">/</div>}
                </button>
              ))}
            </div>
          )}
