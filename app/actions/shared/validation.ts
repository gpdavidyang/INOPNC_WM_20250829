...
export function validateDateTimeWithKST(dateTimeString: string): ValidationResult {
  try {
    ...
    let kstTime: Date
    if (
      dateTimeString.includes('+09:00') ||
      (dateTimeString.includes('T') && dateTimeString.endsWith('Z'))
    ) {
...
export function validateSiteLocation(location: {
  ...
  const hasValidRegion = koreanRegions.some(region => location.address.includes(region))
  ...
  if (location.latitude !== undefined && location.longitude !== undefined) {
    ...
    isInKorea =
      location.latitude >= koreaLat.min &&
...
export function validateDocumentMetadata(metadata: {
  ...
  const hasKoreanText =
    koreanPattern.test(metadata.title) ||
