...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (role_mappings !== undefined) {
      await supabase.from('required_documents_by_role').delete().eq('document_type_id', params.id)
    ...
    if (site_customizations !== undefined) {
      await supabase.from('site_required_documents').delete().eq('document_type_id', params.id)
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabase.from('required_document_types').delete().eq('id', params.id)
