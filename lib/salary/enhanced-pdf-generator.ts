...
abstract class EmploymentTypePDFTemplate {
  ...
  protected drawHeader() {
    ...
    this.doc.text(`발급일: ${this.data.additional_info?.issued_date || new Date().toLocaleDateString('ko-KR')}`, 140, 46)
  ...
  protected drawSalaryDetails() {
    ...
    if (salary_calculation.overtime_pay > 0) {
      this.drawTableRow(['연장수당', '', `${salary_calculation.overtime_pay.toLocaleString()}원`], currentY)
      currentY += 6
    }
    
    const bonus_pay = salary_calculation.tax_details?.bonus_pay || 0
    if (bonus_pay > 0) {
      this.drawTableRow(['상여금', '', `${bonus_pay.toLocaleString()}원`], currentY)
      currentY += 6
    }
    
    // 총 지급액
    this.doc.setFont('helvetica', 'bold')
    this.drawTableRow(['총 지급액', '', `${salary_calculation.gross_pay.toLocaleString()}원`], currentY)
    currentY += 8
  ...
  protected drawFooter() {
    ...
    if (this.data.additional_info?.issued_by) {
      this.doc.text(`발급자: ${this.data.additional_info.issued_by}`, 105, startY + 6, { align: 'center' })
    }
  }
  ...
  protected drawDeductionDetails(startY: number): number {
    ...
    if (deductions.other_deductions > 0) {
      this.drawTableRow([
        '기타 공제',
        '',
        `${deductions.other_deductions.toLocaleString()}원`
      ], currentY)
      currentY += 6
    }
    ...
    this.drawTableRow([
      '총 공제액',
      '',
      `${salary_calculation.total_tax.toLocaleString()}원`
    ], currentY)
    currentY += 10
 ...
 * 고용형태별 PDF 생성 팩토리 함수
 */
export function generateEnhancedSalaryPDF(
  data: EnhancedSalaryPDFData
): Uint8Array {
 ...
 * PDF 다운로드 함수
 */
export function downloadEnhancedSalaryPDF(
  data: EnhancedSalaryPDFData,
  filename?: string
) {
 ...
 * PDF Blob 생성 (미리보기용)
 */
export function getEnhancedSalaryPDFBlob(
  data: EnhancedSalaryPDFData
): Blob {
  const pdfBuffer = generateEnhancedSalaryPDF(data)
...
export function createSamplePDFData(
  ...
  const sampleCalculation: EnhancedSalaryCalculationResult = {
    ...
    total_tax: employment_type === 'regular_employee' ? 22635 : 
                employment_type === 'daily_worker' ? 11880 : 6534,
    net_pay: employment_type === 'regular_employee' ? 157365 :
             employment_type === 'daily_worker' ? 168120 : 173466,
    tax_details: {
      labor_hours: 1.2,
      bonus_pay: 0,
