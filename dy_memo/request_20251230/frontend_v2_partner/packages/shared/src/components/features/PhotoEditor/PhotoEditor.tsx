...
const PhotoEditor: React.FC<PhotoEditorProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-[#121212] z-50 flex flex-col text-white">
      <header className="h-14 bg-black border-b border-[#222] flex items-center justify-between px-3 shrink-0">
      ...
      <div className="bg-[#1e1e1e] p-3 border-b border-[#2a2a2a] flex flex-col gap-2 shrink-0">
        <div className="flex gap-2">
          <input
      ...
      >
        ...
        >
          {Array.from({ length: Math.ceil(gridData.length / 6) }).map((_, pageIdx) => (
            <div
              key={pageIdx}
              className="pe-page w-[794px] h-[1123px] bg-white text-black p-10 shadow-xl flex flex-col box-border pointer-events-auto"
            >
              <div className="text-center mb-4">
                <h1 className="text-3xl font-black">{meta.site || '공사 사진대지'}</h1>
              </div>
              <table className="w-full border-collapse border-2 border-[#333] table-fixed mb-0">
                <tbody>
                  <tr>
                    <td className="border border-[#333] bg-slate-100 font-bold w-[15%] h-[30px] text-center">
                      공사명
                    </td>
                    <td className="border border-[#333] pl-2">{meta.project}</td>
                  </tr>
                </tbody>
              </table>

              <table className="w-full border-collapse border-2 border-[#333] table-fixed mt-[-2px]">
                <colgroup>
                  <col width="15%" />
                  <col width="35%" />
                  <col width="15%" />
                    ...
                    return (
                      <React.Fragment key={row}>
                        <tr className="h-[215px]">
                          <td
                            colSpan={2}
                            className="border border-[#333] p-0 relative bg-slate-50 cursor-pointer overflow-hidden"
                          >
                            {d1 ? (
                        ...
                        <tr>
                          <td className="border border-[#333] bg-slate-100 font-bold text-center h-8">
                            부재명
                          </td>
                          <td className="border border-[#333]">
                            <input
                          ...
                          <td className="border border-[#333] bg-slate-100 font-bold text-center">
                            부재명
                          </td>
                          <td className="border border-[#333]">
                            <input
                        ...
                        <tr>
                          ...
                          <td className="border border-[#333] text-center font-semibold cursor-pointer">
                            <input
                              className="w-full h-full text-center border-none outline-none bg-transparent focus:bg-blue-50"
                              value={d1?.desc || ''}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#222] px-6 py-2 rounded-full flex gap-6 border border-[#333] shadow-2xl z-50">
        <label className="flex flex-col items-center gap-1 text-[10px] text-white/70 hover:text-white cursor-pointer">
          <ImagePlus size={20} />
          <span>추가</span>
          <input type="file" multiple accept="image/*" className="hidden" onChange={handleUpload} />
        </label>
        <div className="w-px h-6 bg-[#444] self-center"></div>
        <button
