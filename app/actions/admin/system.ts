...
function ensureSystemLevelAccess(auth: SimpleAuth) {
  if (auth.isRestricted) {
    throw new AppError('시스템 설정에 접근할 권한이 없습니다.', ErrorType.AUTHORIZATION, 403)
  }
}
...
export async function getSystemConfigurations(
  ...
  return withAdminAuth(async (supabase, profile) => {
    try {
      ensureSystemLevelAccess(profile.auth)
  ...
  return withAdminAuth(async (supabase, profile) => {
    try {
      ...
      if (data_type === 'number') {
        ...
        processedValue =
          typeof setting_value === 'string' ? JSON.parse(setting_value) : setting_value
      }
...
export async function getSystemStats(): Promise<AdminActionResult<SystemStats>> {
  return withAdminAuth(async (supabase, profile) => {
    try {
      ...
      // Get user statistics
      const { data: userStats } = await supabase.from('profiles').select('id, last_seen_at')

      const totalUsers = userStats?.length || 0
      const activeUsers =
        userStats?.filter((u: unknown) => {
      ...
      // Get site statistics
      const { data: siteStats } = await supabase.from('sites').select('id, is_active')

      const totalSites = siteStats?.length || 0
      const activeSites = siteStats?.filter((s: unknown) => s.is_active).length || 0

      // Get document statistics (prefer unified_documents, fallback to documents)
      let totalDocuments = 0
      let storageUsed = 0
      try {
        const { data: unifiedDocs, error: unifiedErr } = await supabase
          .from('unified_documents')
          .select('id, file_size')

        if (!unifiedErr && Array.isArray(unifiedDocs)) {
          totalDocuments = unifiedDocs.length
          storageUsed = unifiedDocs.reduce(
            (sum: number, doc: any) => sum + (Number(doc?.file_size) || 0),
            0
          )
        } else {
          const { data: legacyDocs } = await supabase.from('documents').select('id, file_size')
          totalDocuments = legacyDocs?.length || 0
          storageUsed = (legacyDocs || []).reduce(
            (sum: number, doc: any) => sum + (Number(doc?.file_size) || 0),
            0
          )
        }
      ...
      const { data: reportStats } = await supabase.from('daily_reports').select('id')
      ...
      for (const task of tasks) {
        switch (task) {
          case 'cleanup_old_logs': {
            ...
            await supabase.from('audit_logs').delete().lt('timestamp', ninetyDaysAgo)
  ...
  return withAdminAuth(async (supabase, profile) => {
    try {
      ...
      const uniqueCategories = Array.from(
        new Set(categories?.map((c: unknown) => c.category) || [])
      ) as string[]
