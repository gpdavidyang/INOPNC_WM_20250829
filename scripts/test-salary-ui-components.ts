...
async function testDailySalaryCalculationData() {
  ...
  // 차트 데이터 생성 가능성 검증
  const siteStats = assignmentsData.reduce(
    (acc, a) => {
...
async function testSalaryStatsDashboardData() {
  ...
  // 월별 그룹핑 테스트
  const monthlyStats = assignmentsData.reduce(
    (acc, a) => {
...
async function testPayslipGenerationData() {
  ...
  // 사이트 정보 확인
  const { data: sitesData } = await supabase.from('sites').select('id, name').limit(1)
...
async function testExcelExportData() {
  ...
  const excelData = assignmentsData.map(item => ({
    ...
    총급여:
      (Number(item.labor_hours) || 0) * 15000 +
      Math.max(0, (Number(item.labor_hours) || 0) - 8) * 15000 * 1.5,
  }))
...
async function testChartDataGeneration() {
  ...
  // 작업자별 차트 데이터
  const workerStats = assignmentsData.reduce(
    (acc, a) => {
