...
export default function SitePartnersTab({ siteId, siteName }: SitePartnersTabProps) {
  ...
  const getCompanyTypeBadge = (type: string) => {
    const config = {
      ...
      supplier: { label: '시공업체', color: 'purple' },
    ...
    return (
      <span
        className={`inline-flex px-2 py-1 text-xs font-medium rounded-full bg-${typeConfig.color}-100 text-${typeConfig.color}-800 dark:bg-${typeConfig.color}-900/20 dark:text-${typeConfig.color}-400`}
      >
        {typeConfig.label}
      </span>
    )
  ...
  const getRelationshipBadge = (type: string) => {
    ...
    return (
      <span
        className={`inline-flex px-2 py-1 text-xs font-medium rounded-full bg-${relationConfig.color}-100 text-${relationConfig.color}-800 dark:bg-${relationConfig.color}-900/20 dark:text-${relationConfig.color}-400`}
      >
  ...
  const assignPartner = async (partnerId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setSearchTerm('')
        setTypeFilter('all')
        alert('시공업체가 성공적으로 배정되었습니다.')
      } else {
        const errorData = await response.json()
        console.error('Assignment error response:', errorData)
        alert(errorData.error || '시공업체 배정에 실패했습니다.')
      }
    } catch (error) {
      console.error('Error assigning partner:', error)
      alert('시공업체 배정 중 오류가 발생했습니다.')
    } finally {
  ...
  const unassignPartner = async (partnerId: string, partnerName: string) => {
    if (!confirm(`정말로 '${partnerName}' 시공업체를 현장에서 해제하시겠습니까?`)) {
      return
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            {siteName} 현장의 시공업체 정보와 기성청구 관련 문서를 관리합니다
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">총 시공업체</p>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-gray-100">
                {partnersByType.subcontractor || 0}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <h4 className="text-lg font-medium text-gray-900 dark:text-gray-100">시공업체 정보</h4>
        </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400 text-xs font-medium px-2.5 py-0.5 rounded-full">
                          {partner.site_partners[0]?.contract_status === 'active'
                            ? '활성'
                            : partner.site_partners[0]?.contract_status}
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        {partner.site_partners[0]?.assigned_date && (
                          <div className="flex items-center gap-2">
                            ...
                            배정일:{' '}
                            {format(new Date(partner.site_partners[0].assigned_date), 'yyyy.MM.dd')}
                          </div>
                        )}
                      </div>
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="all">모든 시공업체</option>
                {assignedPartners.map(partner => (
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link
              ...
              className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700"
            >
              <Upload className="h-4 w-4 mr-2" />첫 번째 기성청구서 업로드
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
            {invoiceDocuments.map(document => (
              <div
                key={document.id}
                className="border border-orange-200 dark:border-orange-700 rounded-lg p-4 hover:shadow-md transition-shadow bg-orange-50/50 dark:bg-orange-900/10"
              >
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-500 dark:text-gray-400">
          시공업체 {totalPartners}곳, 기성청구서 {invoiceDocuments.length}건
        </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                      {searchTerm || typeFilter !== 'all'
                        ? '다른 검색 조건을 시도해보세요'
                        : '모든 시공업체가 이미 배정되어 있거나 등록된 시공업체가 없습니다'}
                    </p>
                  </div>
                ) : (
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {unassignedPartners.map(partner => (
                        <tr key={partner.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          ...
                          {/* 업종 */}
                          <td className="px-4 py-3">{getCompanyTypeBadge(partner.company_type)}</td>
                          {/* 대표자 */}
