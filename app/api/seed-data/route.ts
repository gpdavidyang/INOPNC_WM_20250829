...
const SITE_ASSIGNMENTS = [
  ...
  {
    siteName: '서초 B현장',
    workerIds: ['55555555-5555-5555-5555-555555555555', '66666666-6666-6666-6666-666666666666'],
    managerId: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  },
  {
    siteName: '송파 C현장',
    workerIds: ['77777777-7777-7777-7777-777777777777', '88888888-8888-8888-8888-888888888888'],
  ...
  try {
    ...
    if (sitesError) {
      ...
      return NextResponse.json(
        { error: `현장 정보 조회 실패: ${sitesError.message}` },
        { status: 500 }
      )
    }
    ...
    for (const targetName of targetSites) {
      const site = allSites.find((s: unknown) => s.name === targetName)
      if (site) {
        sites.push(site)
      }
    }
    ...
    for (let i = 0; i < assignments.length && i < sites.length; i++) {
      ...
      const site = sites.find((s: unknown) => s.name === assignment.targetName) || sites[i] // 매칭되는 사이트가 없으면 인덱스로 선택

      // 작업자들 배정
      for (const workerId of assignment.workerIds) {
        const assignedDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split('T')[0]

        const { error } = await serviceSupabase.from('site_assignments').upsert(
          {
            site_id: site.id,
            user_id: workerId,
    ...
    for (let i = 0; i < assignments.length && i < sites.length; i++) {
      const assignment = assignments[i]
      const site = sites.find((s: unknown) => s.name === assignment.targetName) || sites[i]
      if (!site) continue
    ...
    for (let i = 0; i < assignments.length && i < sites.length; i++) {
      ...
      for (let j = 0; j < 15; j++) {
        ...
        const issues =
          Math.random() < 0.2
            ? ['날씨로 인한 작업 지연', '자재 배송 지연', '장비 점검 필요', '안전 점검 실시'][
                Math.floor(Math.random() * 4)
              ]
            : null

        const { error } = await serviceSupabase.from('daily_reports').upsert(
          {
            site_id: site.id,
            work_date: workDate.toISOString().split('T')[0],
    ...
    // 작업자들에게 작업 지시 알림
    for (const worker of WORKERS.filter((w: unknown) => w.role === 'worker')) {
      notifications.push({
        user_id: worker.id,
        title: '새로운 작업 지시',
    ...
    // 관리자들에게 검토 알림
    for (const manager of WORKERS.filter((w: unknown) => w.role === 'site_manager')) {
      notifications.push({
        user_id: manager.id,
        title: '작업 일지 검토',
    ...
    return NextResponse.json({
      ...
      results,
      summary: {
        workers: WORKERS.filter((w: unknown) => w.role === 'worker').map(
          (w: unknown) => w.full_name
        ),
        managers: WORKERS.filter((w: unknown) => w.role === 'site_manager').map(
          (w: unknown) => w.full_name
        ),
        sites: sites.map((s: unknown) => s.name),
      },
    })
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    ...
    workers: WORKERS.map((w: unknown) => ({ name: w.full_name, role: w.role })),
    sites: SITE_ASSIGNMENTS.map((s: unknown) => ({
      name: s.siteName,
      workers: s.workerIds.length,
      manager: WORKERS.find((w: unknown) => w.id === s.managerId)?.full_name,
    })),
  })
}
