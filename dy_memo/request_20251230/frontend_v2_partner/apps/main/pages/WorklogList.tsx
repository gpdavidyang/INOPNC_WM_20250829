...
const WorklogList: React.FC = () => {
  ...
  return (
    <div
      className="w-full max-w-[600px] mx-auto min-h-screen relative pb-10"
    ...
    <div onClick={onClick} className={cardClassName}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <button
          ...
          className={`p-1 transition-all shrink-0 ml-auto ${log.isPinned ? 'text-primary rotate-[-45deg]' : 'text-slate-300 hover:text-slate-400'}`}
        >
      ...
      <div className="border-t border-dashed border-[var(--border)] pt-2.5 flex justify-between items-center">
        ...
        <div className="flex gap-1.5 items-center pl-2 border-l border-[var(--border)]">
          <ClipboardList className={`w-4 h-4 ${log.member ? 'text-primary' : 'text-slate-300'}`} />
          <Package className={`w-4 h-4 ${hasMaterials ? 'text-primary' : 'text-slate-300'}`} />
          <Camera className={`w-4 h-4 ${hasPhotos ? 'text-primary' : 'text-slate-300'}`} />
          <MapIcon className={`w-4 h-4 ${hasDrawings ? 'text-primary' : 'text-slate-300'}`} />
          <FileCheckIcon
            className={`w-4 h-4 ${hasConfirmation ? 'text-primary' : 'text-slate-300'}`}
          />
        </div>
      </div>
      ...
      {log.missing && log.missing.length > 0 && (
        <div className="mt-3 bg-sky-50 border border-sky-200 text-sky-700 px-3 py-2 rounded-lg flex items-center gap-2 text-[13px] font-bold">
          <AlertCircle className="w-4 h-4 shrink-0" />
          <span className="truncate">항목 누락</span>
        </div>
      )}
    </div>
  )
        ...
        >
          <div
            className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 transition
                        ${selected ? 'bg-primary border-primary text-white' : 'bg-[var(--bg-surface)] border-[var(--border)]'}
                    `}
          >
