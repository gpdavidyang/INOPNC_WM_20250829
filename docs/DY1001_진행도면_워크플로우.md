...
- 문서함 > 현장공유함(도면 등)(사이트 문서)과 스키마를 공유하여 단일 출처를 사용한다.
...
- 문서함(모바일): `app/mobile/documents/page.tsx` → `app/documents/hub/page.tsx` 재사용
  - 탭: 현장공유함(도면 등)(업로드/목록) `DrawingsTab`
...
3. 분기 처리
   ...
   - 최신 진행도면 없음
     - 안내 메시지 + CTA 버튼 2개
       - ‘도면 업로드’ → `/mobile/documents`의 현장공유함(도면 등) 탭으로 이동(현장=현재, 카테고리=진행 프리셋)
       - ‘도면마킹 도구로 이동’(선택) → `mode=upload`로 마킹툴을 열어 업로드→마킹→게시(게시 시 진행도면 자동 등록)

권장 안내 문구(없을 때):

- “해당 현장의 진행도면이 등록되어 있지 않습니다. 문서함 > 현장공유함(도면 등)에서 ‘진행도면’을 업로드해 주세요.”
...
- 문서함(현장공유함(도면 등)) 초기 프리셋: `app/documents/hub/page.tsx` (DrawingsTab)
...
- 현장공유함(도면 등): `app/documents/hub/page.tsx` (DrawingsTab)
- 도면 목록 API: `app/api/docs/drawings/route.ts`
