...
async function testDocumentSystemComprehensively() {
  ...
  documents?.forEach(doc => {
    ...
    if (
      !categoryStats[category].latestDocument ||
  ...
  sortedCategories.forEach(stat => {
    ...
    const latestTitle = latest
      ? (latest.title || latest.file_name).substring(0, 15) + '...'
      : '없음'
    const latestDate = latest ? new Date(latest.created_at).toLocaleDateString('ko-KR') : '-'
    const status = stat.count > 0 ? '✅ 정상' : '📭 비어있음'

    console.log(
      `| ${stat.koreanName.padEnd(12)} | ${stat.count.toString().padStart(6)} | ${stat.sites.toString().padStart(6)} | ${latestTitle.padEnd(10)} | ${status} |`
    )
  ...
  const pendingCount =
    documents?.filter(doc => !doc.approved_by && doc.status !== 'draft').length || 0
  ...
  console.log(
    `  • 문서 카테고리: ${totalCategories}개 (활성: ${activeCategories}개, 비활성: ${emptyCategories}개)`
  )
  ...
  const siteCoverage = (
    uniqueSites > 0
      ? ((documents?.filter(d => d.sites?.name).length || 0) / totalDocuments) * 100
      : 0
  ).toFixed(1)
  const uploaderCoverage =
    ((documents?.filter(d => d.uploader?.full_name).length || 0) / totalDocuments) * 100
  const titleCoverage =
    ((documents?.filter(d => d.title && d.title.trim().length > 0).length || 0) / totalDocuments) *
    100
