...
export const MainPage: React.FC = () => {
  ...
  return (
    ...
    >
          ...
          {quickMenuItems.map(item => (
            <div
              ...
              onClick={() => {
                if (item.path.startsWith('http')) {
                  ...
                  if ((item as any).state) navigate(item.path, { state: (item as any).state })
            ...
            >
              <div className="qm-icon-wrapper">
                ...
                <img src={item.icon} alt={item.label} className="qm-main-icon" />
              </div>
        ...
        <div className="mb-3">
          <div className="relative">
            <input
              ...
              className="w-full h-[54px] bg-bg-input dark:bg-slate-900 border border-border dark:border-slate-600 rounded-xl px-4 text-[17px] font-medium outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
            ...
            {showSiteDropdown && (
              <ul className="absolute z-50 w-full mt-1.5 max-h-60 overflow-auto bg-[var(--bg-surface)] border border-border dark:border-slate-600 rounded-xl shadow-xl animate-[slideDown_0.2s_ease-out]">
                    ...
                    <li
                      ...
                      className={`px-4 py-3.5 cursor-pointer text-[15px] font-medium border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700 ${
                        selectedSite === s.value
                          ? 'text-primary bg-blue-50 dark:bg-slate-700/50'
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full h-[54px] bg-bg-input dark:bg-slate-900 border border-border dark:border-slate-600 rounded-xl px-4 text-[17px] font-medium outline-none focus:border-primary cursor-pointer"
                ...
                <div className="relative w-full min-w-0">
                  {!item.isCustom ? (
                    <select
                      ...
                      className="w-full h-[50px] bg-[var(--bg-surface)] dark:bg-slate-900 border border-border dark:border-slate-600 rounded-xl px-4 appearance-none outline-none focus:border-primary text-[15px] font-medium"
                      style={{
                    ...
                    <input
                      ...
                      className="w-full h-[50px] bg-[var(--bg-surface)] dark:bg-slate-900 border border-border dark:border-slate-600 rounded-xl px-4 outline-none focus:border-primary text-[15px] font-medium placeholder:text-slate-400 dark:placeholder:text-slate-500"
                    />
                  )}
                </div>
              )}

              <div className="flex h-[48px] border border-border dark:border-slate-600 rounded-xl bg-[var(--bg-surface)] dark:bg-slate-900 overflow-hidden w-full min-w-0">
                <button
      ...
      <div className="bg-bg-surface dark:bg-slate-800 rounded-2xl p-6 mb-4 shadow-sm border border-transparent dark:border-slate-700">
        ...
        <div className="flex flex-col gap-4">
          {workSets.map((ws, idx) => (
            <div
              key={ws.id}
              className="bg-[var(--bg-surface)] dark:bg-slate-900 border-2 border-primary rounded-2xl p-5 animate-[slideDown_0.3s_ease-out]"
            >
      ...
      {/* Floating Actions */}
      <div
        className={`sticky bottom-[80px] bg-bg-surface dark:bg-slate-800 p-2.5 rounded-2xl border border-border shadow-xl mb-5 z-40 transition-all ${isReadyToSave ? 'border-green-500 ring-4 ring-green-500/10' : ''}`}
      >
        {isReadyToSave && (
        ...
        <div className="flex gap-2.5">
          <button
            onClick={handleReset}
            className="flex-1 h-[50px] bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl border border-slate-300 dark:border-slate-600 text-[15px] sm:text-[16px]"
          >
            초기화
          </button>
          <button
            onClick={handleSave}
            className={`flex-1 h-[50px] text-white font-bold rounded-xl transition-all text-[15px] sm:text-[16px] ${isReadyToSave ? 'bg-green-500 hover:bg-green-600 shadow-[0_0_0_3px_rgba(16,185,129,0.2)]' : 'bg-header-navy hover:bg-slate-800'}`}
          >
            일지저장
          </button>
        </div>
      ...
      <div className="bg-bg-surface dark:bg-slate-800 rounded-2xl p-6 mb-4 shadow-sm border border-transparent dark:border-slate-700">
        ...
        {photos.length > 0 || drawings.length > 0 ? (
          <div className="flex gap-3 overflow-x-auto pb-3 pt-2 mb-3 no-scrollbar px-1">
            {photos.map((p, i) => (
              <div key={`p-${i}`} className="relative group flex-shrink-0">
                <div className="w-16 h-16 rounded-lg bg-slate-100 overflow-hidden border">
                  <img src={p.img} className="w-full h-full object-cover" alt={`사진 ${i + 1}`} />
                </div>
                <button
                  onClick={e => {
                    e.stopPropagation()
                  ...
                  className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold hover:bg-red-600 transition-colors opacity-0 group-hover:opacity-100"
                  title="삭제"
                >
                  ×
                </button>
            ...
            {drawings.map((d, i) => (
              <div key={`d-${i}`} className="relative group flex-shrink-0">
                <div className="w-16 h-16 rounded-lg bg-slate-100 overflow-hidden border">
                  <img
                    src={d}
                    className="w-full h-full object-contain bg-[var(--bg-surface)]"
                    alt={`도면 ${i + 1}`}
                  />
                </div>
                <button
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-teal-500 text-white text-[10px] text-center py-0.5">
                  도면
                </div>
              </div>
            ))}
      ...
      {isDrawingSheetOpen && (
        <>
          ...
          />
          <div className="fixed bottom-0 left-0 right-0 bg-[var(--bg-surface)] border-t border-[var(--border)] rounded-t-2xl p-6 z-[101] animate-[slideDown_0.3s_ease-out] max-w-[600px] mx-auto">
            <div className="text-center font-bold text-lg mb-5 text-[var(--text-main)]">
              도면 선택
            </div>
            <label className="w-full h-[54px] bg-blue-50 text-blue-500 border border-blue-200 rounded-xl font-bold flex items-center justify-center gap-2 mb-2.5 cursor-pointer hover:bg-blue-100 transition-colors">
            ...
            <button
              ...
              className="w-full h-[54px] bg-[var(--bg-hover)] text-[var(--text-main)] border border-[var(--border)] rounded-xl font-bold flex items-center justify-center gap-2 mb-2.5 hover:bg-[var(--bg-input)] transition-colors"
            >
              <Map size={18} /> 현장 도면 불러오기
            </button>
            <button
              onClick={() => setIsDrawingSheetOpen(false)}
              className="w-full h-[54px] bg-header-navy text-white rounded-xl font-bold mt-2 hover:bg-slate-700 transition-colors"
            >
              닫기
            </button>
          </div>
      ...
      {isSiteDrawingViewerOpen && (
        <div className="fixed inset-0 bg-slate-950 z-[60] flex flex-col animate-[fadeIn_0.2s]">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            <div className="max-w-[1200px] mx-auto w-full">
              {siteDrawingPages.map((src, i) => (
                <div
                  key={i}
                  className="bg-[var(--bg-surface)] rounded-xl overflow-hidden shadow-lg mb-4"
                  onClick={() => {
                ...
                >
                  <div className="p-3 border-b border-[var(--border)] flex justify-between items-center bg-[var(--bg-hover)]">
                    <span className="font-bold text-[var(--text-main)]">Page {i + 1}</span>
                  ...
                  <img
                    src={src}
                    className="w-full h-auto object-contain bg-[var(--bg-surface)]"
                    alt={`Page ${i + 1}`}
                  />
                </div>
              ))}
      ...
      <div className="bg-bg-surface dark:bg-slate-800 rounded-2xl p-6 mb-8 shadow-sm border border-transparent dark:border-slate-700">
        ...
        >
          {[
            ...
            <div
              key={i}
              className={`flex justify-between py-1.5 ${i === arr.length - 1 ? '' : 'border-b border-slate-100 dark:border-slate-700'}`}
            >
