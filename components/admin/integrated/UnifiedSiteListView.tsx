...
export default function UnifiedSiteListView() {
  ...
  const fetchSites = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          // Filter out the 'all' option and keep only actual sites
          setSites(data.data.filter((site: Site) => site.id !== 'all'))
        }
      }
    } catch (error) {
      ...
      {sites.length === 0 ? (
        ...
        <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-md">
          <ul className="divide-y divide-gray-200 dark:divide-gray-700">
            {sites.map((site) => (
              <li key={site.id} className="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-700">
                <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-4 mt-1">
                        ...
                        {site.start_date && (
                          <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                            ...
                            {new Date(site.start_date).toLocaleDateString('ko-KR')}
                          </div>
                        )}
                      </div>
                      {site.manager_name && (
