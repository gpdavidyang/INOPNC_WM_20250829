...
export default function AdditionalPhotoUploadSection({
  ...
  const handleFileSelect = useCallback((files: FileList, photoType: 'before' | 'after') => {
  ...
  }, [beforePhotos, afterPhotos, maxPhotosPerType, maxFileSize, onBeforePhotosChange, onAfterPhotosChange])

  const handleDrop = useCallback((e: React.DragEvent, photoType: 'before' | 'after') => {
    e.preventDefault()
    setDragOver(false)
  ...
  const removePhoto = useCallback((photoId: string, photoType: 'before' | 'after') => {
    const currentPhotos = photoType === 'before' ? beforePhotos : afterPhotos
  ...
  const updatePhotoDescription = useCallback((photoId: string, description: string, photoType: 'before' | 'after') => {
    const currentPhotos = photoType === 'before' ? beforePhotos : afterPhotos
  ...
  const renderPhotoGrid = useCallback((photos: AdditionalPhotoData[], photoType: 'before' | 'after') => {
    if (photos.length === 0) {
      return (
    ...
    <div className="space-y-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
      ...
      <div className="flex items-center gap-3 pb-2 border-b border-gray-100 dark:border-gray-700">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">작업전후 사진을 별도로 관리할 수 있습니다</p>
        </div>
      </div>
      ...
      >
        <div className="text-center">
          <div className="flex items-center justify-center gap-4 mb-4">
            <div className={`p-3 rounded-full ${dragOver ? 'bg-blue-100 dark:bg-blue-900/40' : 'bg-gray-100 dark:bg-gray-700'} transition-colors`}>
            ...
            <>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 text-xs text-gray-500 dark:text-gray-400">
                <span className="flex items-center gap-1">
                  ...
                  <span className="hidden sm:inline">최대 </span>{remainingSlots}장<span className="hidden sm:inline"> 더 업로드 가능</span>
                </span>
                <span className="flex items-center gap-1">
                ...
                <span className="flex items-center gap-1">
                  ...
                  <span className="hidden sm:inline">파일당 </span>최대 {Math.round(maxFileSize / 1024 / 1024)}MB
                </span>
              </div>
            </>
          ) : (
      ...
      <div className="min-h-[200px]">
        {renderPhotoGrid(currentPhotos, activeTab)}
      </div>
