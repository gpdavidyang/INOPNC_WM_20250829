...
async function testDocumentIntegrationAPI() {
  ...
  try {
    ...
    if (data.documents_by_category) {
      ...
      for (const [category, documents] of Object.entries(data.documents_by_category)) {
        ...
        if (docs && docs.length > 0) {
          ...
          console.log(
            `   👤 업로더: ${sample.uploader?.full_name || sample.profiles?.full_name || '알 수 없음'}`
          )
    ...
    console.log('\n📊 통계 정확성 검증:')
    const calculatedTotal = Object.values(data.documents_by_category || {}).reduce(
      (sum, docs) => sum + (docs as any[]).length,
      0
    )
    ...
    for (const category of ['shared', 'markup', 'required']) {
      try {
        const filterResponse = await fetch(
          `http://localhost:3002/api/admin/documents/integrated?category_type=${category}`,
          {
            headers: {
              Authorization: `Bearer ${authData.session.access_token}`,
              'Content-Type': 'application/json',
    ...
    try {
      const searchResponse = await fetch(
        `http://localhost:3002/api/admin/documents/integrated?search=test`,
        {
          headers: {
            Authorization: `Bearer ${authData.session.access_token}`,
            'Content-Type': 'application/json',
