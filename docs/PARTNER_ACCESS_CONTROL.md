...
건설 현장 관리 시스템에서 시공업체(협력업체)의 데이터 접근을 제어하고 격리하는 시스템입니다.

### 핵심 원칙

- **오직 `customer_manager` 역할만** 시공업체 기반 데이터 필터링 적용
...
| **customer_manager** | 시공업체 관리자 | **자사 데이터만** | partner_company_id 필수 |
...
VALUES ('새시공업체', '123-45-67890', 'active');

-- 2. customer_manager 사용자 생성 시 partner_company_id 설정
UPDATE profiles
SET partner_company_id = '새시공업체_ID'
