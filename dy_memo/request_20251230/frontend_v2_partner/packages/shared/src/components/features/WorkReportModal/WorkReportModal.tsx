...
export interface WorkReportModalProps {
  isOpen: boolean
  onClose: () => void
  siteName: string
...
const WorkReportModal: React.FC<WorkReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-[#1e1e1e] z-50 flex flex-col text-white">
      <header className="h-[60px] bg-black border-b border-[#333] flex items-center justify-between px-4 shrink-0">
        <button onClick={onClose}>
          <ChevronLeft size={28} />
        </button>
        <span className="text-lg font-bold text-white">작업완료확인서</span>
      ...
      <div className="flex-1 overflow-auto bg-[#1e1e1e] flex justify-center pt-5 pb-20">
        <div
        ...
        >
          <div
            id="wr-document-area"
            key={formKey}
            className="w-[210mm] min-h-[297mm] bg-white text-black p-[15mm] shadow-2xl flex flex-col box-border"
          >
            ...
            <div className="mt-4 text-center">
              ...
              <div className="grid grid-cols-[33%_27%_40%] border-2 border-slate-800 mb-5">
                ...
                <div className="relative bg-white h-[200px] flex flex-col">
                  ...
                  <div className="h-11 bg-slate-50 border-t border-slate-200 flex justify-between items-center px-2">
                    <button
                      onClick={handleClear}
                      className="text-xs px-3 py-1 bg-white border border-slate-300 rounded text-slate-600 font-semibold shadow-sm"
                    >
                      지우기
                    </button>
                  </div>
