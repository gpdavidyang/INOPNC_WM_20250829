  ...
  try {
    ...
    if (profile.role !== 'admin') {
      if (profile.role === 'supervisor') {
        documentsQuery = documentsQuery.or(`is_public.eq.true,owner_id.eq.${user.id},uploaded_by.eq.${user.id}`)
      } else {
        documentsQuery = documentsQuery.or(`owner_id.eq.${user.id},uploaded_by.eq.${user.id},is_public.eq.true`)
      }
    }
